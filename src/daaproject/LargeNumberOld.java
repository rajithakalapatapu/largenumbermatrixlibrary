...
public class LargeNumberOld {
	...
	public LargeNumberOld() {
	}

	public LargeNumberOld(int length) {
		digits = new int[length];
	}

	public LargeNumberOld(String largeNumberAsString) {
	...
	public LargeNumberOld add(LargeNumberOld other) {
		LargeNumberOld result = new LargeNumberOld(Math.max(this.numberOfDigits(), other.numberOfDigits()));
...
//			System.out.println("i - " + i + " j - " + j + " k - " + k + " digits " + sum % 10 + " carry " + carry);
		...
		if (carry > 0) {
			LargeNumberOld biggerResult = new LargeNumberOld(result.numberOfDigits() + 1);
